...
from .modeling.sam_criterion import SetCriterion
...
from .modeling.lora import LoRA_Sam
...
class MaskFormer(nn.Module):
    ...
    def __init__(
       ...
       # self.test_proposal_sampler_1=NLSSampler(thresh=-0.15, ignore_mask=None, method="kmeans", num_points=50)
        ...
        self.nms_thresh = 0.7
        self.ignore_mask = torch.load(self.ignore_mask_path).cuda()
        ...
        #mask_in_chans = 16
        #embed_dim = 256
        self.mask_downscaling = self.sam.prompt_encoder.mask_downscaling
        #self.mask_downscaling_id = nn.Sequential(
        #        nn.Conv2d(19, 16 // 4, kernel_size=2, stride=2),
        #        self.mask_downscaling[1],
        #        self.mask_downscaling[2],
        #        self.mask_downscaling[3],
        #        self.mask_downscaling[4],
        #        self.mask_downscaling[5],
        #        self.mask_downscaling[6],
        #        )
    ...
    def from_config(cls, cfg):
        ...
        losses = ["sam"]#, "sam_iou"]#, "sam_bce"]#, "sam_dice"]#[ "labels", "objects" ,"masks"]#, "obj_mask", "obj_labels"]["contrastive",
    ...
    def forward(self, batched_inputs, include_void=False, return_separately=False, 
        ...
        if self.training:
            ...
            for i in range(ood_train.shape[0]):
                ood_train[i] = ood_train[i] - torch.min(ood_train[i], dim=0)[0]
            #print(torch.min(ood))
            #print(torch.max(ood))
            inlier = sem
            #con_map = []
	    ...
	    #)
            ...
            ood_masks = torch.sum(ood_masks, dim=1)
            ood_masks = retry_if_cuda_oom(sem_seg_postprocess)(
                                   ood_masks.float(), (ood_masks.shape[-2], ood_masks.shape[-1]), 1024, 2048)
            points, train_masks = self.fine_with_sam_train(images_t,  ood_masks, inlier, f_feature)
    ...
    def sam_output(self, x, query_coordinates,  uncertainty, o_feature):
        ...
        for i in range(x.shape[0]):
            ...
            #align_corners=False,
        #)
            #mask_prompt = torch.where(mask_prompt > torch.min(mask_prompt)+ 0.7*(torch.max(mask_prompt) - torch.min(mask_prompt)), torch.tensor(True, device=nls.device), torch.tensor(False, device=nls.device) )
            (
                sparse_embeddings,
                dense_embeddings,
            ...
            sam_features = self.predictor.features
    ...
    def fine_with_sam_train(self, x, uncert, inlier_output, o_feature):
        ...
        return torch.stack(points), ood_output["train_masks"]
