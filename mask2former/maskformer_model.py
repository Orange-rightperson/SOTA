...
def compute_distance_map(road_mask):
    """
    计算每个像素到 road 区域的最近距离。

    :param road_mask: (H, W) 的 PyTorch Tensor，二值化，1 表示 road，0 表示非 road。
    :return: (H, W) 的 PyTorch Tensor，每个像素到最近 road 像素的欧几里得距离。
    """
    road_mask_np = road_mask.cpu().numpy().astype(np.uint8)

    # 计算非 road 区域到 road 的最近距离
    distance_map = distance_transform_edt(road_mask_np == 0)

    return torch.from_numpy(distance_map).to(road_mask.device)
...
class ConvNet(nn.Module):
    def __init__(self, mask_in_chans, embed_dim, cls_num):
        super(ConvNet, self).__init__()
        # 定义卷积层，将通道数从 20 -> 256，尺寸从 256x512 -> 64x64
        self.conv1 = nn.Sequential(
            nn.Conv2d(cls_num, mask_in_chans // 4, kernel_size=2, stride=2),
...
class MaskFormer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.test_proposal_sampler=NLSSampler(thresh=-0.15, ignore_mask=None, method="fps", num_points=0)
        self.test_proposal_sampler_1=NLSSampler(thresh=0, ignore_mask=None, method="fps", num_points=0)
        ...
        embed_dim = 256
        self.num = 2
        self.mask_downscaling = ConvNet(mask_in_chans, embed_dim, 1)#deepcopy(self.sam.prompt_encoder.mask_downscaling)
        self.ood_attention = ConvNet(mask_in_chans, embed_dim, 1)#deepcopy(self.sam.prompt_encoder.mask_downscaling)
        #self.road_attention = ConvNet(mask_in_chans, embed_dim, 1)
    ...
    def forward(self, batched_inputs, include_void=False, return_separately=False, 
        ...
        features = self.backbone(images.tensor)#reshape_images)#reshape_images)#reshape_images)
	    ...
	    #)
            ...
            train_mask = retry_if_cuda_oom(sem_seg_postprocess)(
                    train_mask.float(), (train_mask.shape[-2],train_mask.shape[-1]), images.tensor.shape[-2] ,images.tensor.shape[-1])
            ood = retry_if_cuda_oom(sem_seg_postprocess)(
                                        ood, (ood.shape[-2],ood.shape[-1]), images.tensor.shape[-2] ,images.tensor.shape[-1])
            points = self.app_coords(points, images.tensor.shape[-2], images.tensor.shape[-1] )
            pred = {"train_masks": train_mask, "points": points, "rba": ood}
            ...
            sem = retry_if_cuda_oom(self.semantic_inference)(
                                                mask_cls_results[0], mask_pred_results[0])
            ood = retry_if_cuda_oom(self.nls_ood_inference)(
                                                mask_cls_results[0], mask_pred_results[0])
            #for i in range(ood.shape[0]):
            #    ood[i] = ood[i] - torch.min(ood[i])
            ood_train = F.interpolate(
                ood.unsqueeze(0),
                size=(256, 256),
                mode="bilinear",
            ...
            for mask_cls_result,   mask_pred_result, input_per_image, image_size in zip(
                ...
                if self.semantic_on:
                    ...
                    ood = retry_if_cuda_oom(sem_seg_postprocess)(
                            ood, image_size, height, width)
                    inlier = retry_if_cuda_oom(sem_seg_postprocess)(
                            inlier, image_size, height, width)
                    #print(inlier.shape)
                    #r = retry_if_cuda_oom(self.batched_semantic_inference)(
                     #                                   mask_cls_result, mask_pred_result)
                    #r = retry_if_cuda_oom(sem_seg_postprocess)(
                     #                                   r, image_size, height, width)
                    #print(r.shape)
                    processed_results[-1]["sem_seg"] = inlier
                    #ood = gt            
                    ...
                    #train_masks = retry_if_cuda_oom(sem_seg_postprocess)(
                    #        torch.stack(train_masks).float(), (1024,2048), height ,width)
    ...
    def set_requires_grad(self, nets, requires_grad=False):
        ...
        for param in nets.mask_decoder.output_hypernetworks_mlps.parameters():#parameters():#.output_hypernetworks_mlps.parameters():
            param.requires_grad = True
        for param in nets.mask_decoder.iou_prediction_head.parameters():
    ...
    def sam_output(self, x, query_coordinates,  uncertainty, o_feature, road_prediction):
        ...
        for i in range(x.shape[0]):
            ...
            labels = torch.zeros_like(points[1])

            sparse_embeddings = self.predictor.model.prompt_encoder._embed_points(points[0], labels, pad=(None))
            sparse_embeddings = sparse_embeddings.permute(1, 0, 2)

            road = road_prediction[i]
            road_mask = retry_if_cuda_oom(sem_seg_postprocess)(
                            road.unsqueeze(0).unsqueeze(0), (road.shape[-2],road.shape[-1]), 1024 ,2048)
            road_mask =  F.interpolate(
            road_mask.unsqueeze(0),
            size=(256, 256),
            mode="bilinear",
            align_corners=False,
        )
            mask_prompt = retry_if_cuda_oom(sem_seg_postprocess)(
                        nls.unsqueeze(0).unsqueeze(0), (road.shape[-2],road.shape[-1]), 1024 ,2048)
            mask_prompt = F.interpolate(
            mask_prompt.unsqueeze(0),
            size=(256, 256),
            mode="bilinear",
            align_corners=False,
        )
            ...
            extra_embeddings = self.road_attention(road_mask)
            ...
            dense_embeddings = ood_embeddings
           ...
           # dense_embeddings = nn.functional.gelu(dense_embeddings)
            ...
            #features = o_feature[0] + o_feature[1]
            ...
            #area = (mask > 0).sum(dim=(2, 3))
            #max_idx = area.argmax(dim=1) 
            mask = F.interpolate(
                        mask,
                        size=(uncertainty.shape[-2], uncertainty.shape[-1]),
                        mode="bilinear",
                        align_corners=False,
                )
            mask = mask > 0#0.5#self.predictor.model.mask_thresholdi
            #mask = torch.sum(mask, dim=1, keepdim=True)

            #mask = torch.sum(mask, dim=1, keepdim=True)
            mask = (mask * road).bool()
            #mask = mask[torch.arange(mask.shape[0]), max_idx].unsqueeze(1)
            masks.append(mask.flatten(0, 1))
            score = []
            for i in range(mask.shape[0]):
                #nls_mean = compute_distance_map(road)*nls 
                nls_mean = (1-road)*nls
                nls_score = nls[mask.squeeze(1)[i]].mean()  # 提取符合 mask 的值
                if nls_score.isnan():  # 如果为空，返回 -1
                    nls_score = -torch.ones_like(nls_score)#torch.tensor(-1.0, device=nls.device, dtype=nls.dty
                #nls_score =nls[mask.squeeze(1)[i]].max()
                #if nls_score.isnan():
    ...
    def dilate_mask(self, mask, kernel_size=15, iterations=15):
            # 创建卷积核
    ...
    def fine_with_sam_train(self, x, uncert, inlier_output, o_feature):
        ...
        (
            ...
            _,
            _,
        ) = self.test_proposal_sampler_1(x,  inlier_output.max(0)[0], inlier_output)
    ...
    def fine_with_sam(self, x, uncert, inlier_output, o_feature):
        ...
        road_prediction = (inlier_output.max(0)[1] == 0).unsqueeze(0).float()
        road_prediction = self.dilate_mask(road_prediction).squeeze(0)
        #print(query_coordinates[0].shape)
        #exit()
        ...
        for i in range(len(ood_output["pred_logits"])):
            ...
            sam_mask = torch.sum(masks,  dim=0)
        ...
        if self.minimal_uncert_value_instead_zero:
            uncertainty[uncertainty == 0] = -1#0.5 *(torch.min(nls_uncert)+torch.max(nls_uncert))#-1#self.minimal_uncert_value_instead_zero
        #nls_uncert = torch.stack(ood_output["uncert"])
        # print(torch.min(uncertainty))
        # print(torch.min(nls_uncert))
        #dm = (1-compute_distance_map(road_prediction)).cpu()
        uncertainty =  nls_uncert+uncertainty#torch.pow(dm, 3)*nls_uncert#(uncertainty+nls_uncert)#+
