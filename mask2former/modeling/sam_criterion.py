...
class SetCriterion(nn.Module):
    ...
    def sam_dice_loss(self, pred, targets, indices, num_masks):
        #_, src_idx = self._get_src_permutation_idx(indices)
        #_, tgt_idx = self._get_tgt_permutation_idx(indices)
        outlier_masks = torch.cat([x["outlier_masks"].unsqueeze(
                                0) for x in targets], dim=0)
        points = torch.floor(pred["points"]).long()
        H, W = (512, 1024)
        C=outlier_masks.shape[1]
        batch_size = points.shape[0]
        num_points = points.shape[1]
        target_o = outlier_masks
        points = points.squeeze(2)
        #points = torch.where(points >= 1023, 1023,points)
        indexes = []
        # 将 points 分解为行索引和列索引
        y_indices = points[:, :, 0]  # (batch_size, num_points)
        #print(points.shape)
        x_indices = points[:, :, 1]  # (batch_size, num_points)
        y_indices = torch.clamp(y_indices, 0, H-1)  # 限制 y 坐标范围
        x_indices = torch.clamp(x_indices, 0, W-1)
        #y_indices = y_indices.unsqueeze(-1).expand(-1, -1, C)  # (1, 50, 4)
        #x_indices = x_indices.unsqueeze(-1).expand(-1, -1, C)  # (1, 50, 4)
        #point_values = target_o[0, :, y_indices.squeeze(0), x_indices.squeeze(0)]
        point_values = target_o.gather(
                    dim=2, index=y_indices.unsqueeze(1).unsqueeze(-1).expand(-1, C, -1, W)
                    ).gather(
                               dim=3, index=x_indices.unsqueeze(1).unsqueeze(-1).expand(-1, C, -1, -1)
                                ).squeeze(-1).squeeze(-1)  # (1, 4, 50)
        #indices = torch.max(point_values, dim=1)[1]
                                            # 提取对应的 Mask
        is_all_zero = (point_values.sum(dim=1) == 0)  # (1, 50), 检查每列是否全为 0

        # 找到第二个维度中值为 1 的最大索引
        max_indices = torch.argmax((point_values == 1).int(), dim=1)  # (1, 50)，找到值为 1 的最大索引

        # 如果第二个维度全为 0，返回 0，否则返回最大索引
        indices = torch.where(is_all_zero, torch.tensor(0, device=point_values.device), max_indices)
        #batch_indices = torch.arange(batch_size).unsqueeze(-1)  # (1, 1)    
        target  = target_o[0, indices, :, :].squeeze(0).float()  # (1, 50, H, W) 
        # 利用批量索引提取每个点对应的通道值
        # 提取每个点在 (C, H, W) 中的值，结果为 (batch_size, num_points, C)
        #point_values = target_o[0, :, y_indices, x_indices]
        
        #indexes = []
        #print(points.shape)
        #for i in range(points.shape[1]):
        #    indexes.append(torch.max(target_o[0 , :, y_indices[0,i], x_indices[0,i]]))
        #print(torch.unique(target_o))
        #indexes = torch.max(target_o[0 , :, points], dim=1)
        
        #target = target_o[0, indexes, :, :].float()
        #target = target_o.repeat(50,1,1)
        masks = torch.sigmoid(pred["train_masks"] )
        #print(masks.shape)
        #target=target[tgt_idx]
        #masks=masks[src_idx]
        return {"sam_dice_loss": self._dice_loss(masks, target)}
    ...
    def sam_bce_loss(self, pred, targets, indices, num_masks):
       # _, src_idx = self._get_src_permutation_idx(indices)
       # _, tgt_idx = self._get_tgt_permutation_idx(indices)
        outlier_masks = torch.cat([x["outlier_masks"].unsqueeze(
                                0) for x in targets], dim=0)
        target_o = outlier_masks
        masks = pred["train_masks"]
        #target=target[tgt_idx]
        #masks=masks[src_idx]
        points = torch.floor(pred["points"]).long()
        #print(points.device)
	...
	# 将 points 分解为行索引和列索引
        ...
        #for i in range(points.shape[1]):
        #    indexes.append(torch.max(target_o[0 , :, y_indices[0,i], x_indices[0,i]]))
            #print(target[0, :,index[0,1], index[0,0]])
        #target_o = torch.where(target_o > 1, 1, target_o)
        #print(torch.unique(target_o))
        #target = target_o[0, indexes, :, :].float()
        #target = target_o.repeat(50,1,1)
        #pred = pred["train_masks"]
        #print("bce")
        #print(pred)
        loss = torch.nn.BCEWithLogitsLoss()
        return {"sam_bce_loss": loss(masks, target)}

    def sam_iou_loss(self, pred, targets, indices, num_masks):
        #_, src_idx = self._get_src_permutation_idx(indices)
        #_, tgt_idx = self._get_tgt_permutation_idx(indices)
        outlier_masks = torch.cat([x["outlier_masks"].unsqueeze(
                                0) for x in targets], dim=0)
        ...
        #target=target[tgt_idx]
        #masks=masks[src_idx]
        inter = (masks * target).sum(dim=(1, 2))
        union = (masks + target).sum(dim=(1, 2)) - inter
        iou = 1 - (inter / union)
        return {"sam_iou_loss": iou.mean()}
    ...
    def get_loss(self, loss, outputs, targets, indices, num_masks):
        loss_map = {
            ...
            "sam_iou": self.sam_iou_loss,
            "sam_bce": self.sam_bce_loss,
            "sam_dice": self.sam_dice_loss
        }
        assert loss in loss_map, f"do you really want to compute {loss} loss?"
        return loss_map[loss](outputs, targets, indices, num_masks)

    def forward(self, outputs, targets, train_masks):
...
#        # Retrieve the matching between the outputs of the last layer and the targets
        indices = None#self.matcher(outputs_without_aux, targets)
       # indices = self.matcher(train_masks, targets)
...
#                    losses.update(l_dict)
        ...
        for loss in self.losses:
            losses.update(self.get_loss(
                loss, train_masks, targets, indices, num_masks))
        return losses
