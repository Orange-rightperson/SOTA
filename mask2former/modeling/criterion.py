...
class SetCriterion(nn.Module):
    ...
    def sam_dice_loss(self, pred, targets, indices, num_masks):
        ...
        target = outlier_masks#/255.0
        masks = pred["train_masks"]
        #target = target.repeat(50,1,1)
        masks = masks.unsqueeze(1)
        target = target.unsqueeze(1)
        seg_loss = monai.losses.DiceCELoss(sigmoid=True, squared_pred=True, reduction='mean')
        loss = seg_loss(masks, target)
        return {"sam_dice_loss": loss}
    ...
    def sam_bce_loss(self, pred, targets, indices, num_masks):
       ...
       # masks=masks[src_idx]
        ...
        #    index = points[0,i]
        #    indexes.append(torch.max(target_o[0 , :, index[0,1], index[0,0]]) - 1)
    ...
    def sam_iou_loss(self, pred, targets, indices, num_masks):
        ...
        iou = 1 - (inter / union)
        return {"sam_iou_loss": self._dice_loss(masks, target)}
