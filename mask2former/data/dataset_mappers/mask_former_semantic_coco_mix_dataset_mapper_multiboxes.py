...
class MaskFormerSemanticCocoMixDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        if ood_p < self.ood_prob:
            for i in range(ood_num):
                ood_idx = np.random.randint(0, len(self.coco_dataset))
                ood_object, ood_mask = self.coco_dataset[ood_idx]
                image, sem_seg_gt = mix_object(image, sem_seg_gt, np.array(ood_object), np.array(ood_mask), self.ood_label, i)  
        ...
        if sem_seg_gt is not None:
            ...
            outlier_masks = []
            if ood_p < self.ood_prob:
                id_masks = torch.zeros_like(sem_seg_gt).contiguous()
                id_masks[sem_seg_gt == self.ignore_label] = 1
                outlier_masks.append(id_masks)
                for j in range(ood_num):
                    outlier_mask = torch.zeros_like(sem_seg_gt).contiguous()
                    outlier_mask[(sem_seg_gt == (self.ood_label-i)) & (sem_seg_gt != self.ignore_label)] = 1
                    outlier_mask[sem_seg_gt == self.ignore_label] = 0#self.ignore_label
                    outlier_masks.append(outlier_mask)
                    #outlier_masks = torch.stack(outlier_masks, 0)
            #print(self.ignore_label
            else:
                outlier_mask  = torch.zeros_like(sem_seg_gt).contiguous()
                outlier_mask[sem_seg_gt == self.ignore_label] = 1
                outlier_masks.append(outlier_mask)
            outlier_masks = torch.stack(outlier_masks, 0)
            dataset_dict["outlier_mask"] = outlier_masks.long()
