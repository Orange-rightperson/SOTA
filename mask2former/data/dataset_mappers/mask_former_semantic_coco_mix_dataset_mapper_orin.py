...
def random_resize(image: np.ndarray, mask: np.ndarray, scale_range=(0.05, 0.9)):
    """
    对图像和对应掩码进行随机尺度缩小。
...
class MaskFormerSemanticCocoMixDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        if ood_p < self.ood_prob:
            ...
            image, sem_seg_gt = mix_object(image, sem_seg_gt, ood_object, ood_mask, self.ood_label)   
        ...
        if sem_seg_gt is not None:
            ...
            outlier_mask[sem_seg_gt == self.ignore_label] = 1#self.ignore_label
            dataset_dict["outlier_mask"] = outlier_mask.float()
