...
from datasets.segment_me import SegmentMeIfYouCan
...
from datasets.fs_static import Fs_Static
...
def get_datasets(datasets_folder):
    ...
    fishyscapes_static_config = edict(
        dataset_root=os.path.join(datasets_folder, 'fs_static'),
    )

    road_anomaly_21_config = edict(
        root=os.path.join(datasets_folder,
    ...
    DATASETS = edict(
        ...
        road_anomaly_21 = SegmentMeIfYouCan(root = os.path.join(datasets_folder,
                                            'segment_me'), split='road_anomaly', transform=transform_ra_21),
        road_obstacles=SegmentMeIfYouCan(root = os.path.join(datasets_folder,
                                                        'segment_me'), split='road_obstacle',transform=transform),
        fs_static=Fs_Static(root = os.path.join(datasets_folder,
                                                        'fs_static'), transform=transform),
...
class OODEvaluator:
    ...
    def compute_anomaly_scores(
        ...
        for x, y in tqdm(loader, desc="Dataset Iteration"):
            ...
            score, score_o, point, sam_output = self.get_anomaly_score(x)  # -> (H, W)
            score =   score
            if use_gaussian_smoothing:
            ...
            points.append(point.squeeze(0).squeeze(1).cpu().numpy())
            anomaly_score.extend([score.cpu().numpy()])
            sam_outputs.append(sam_output[0].cpu())
        ...
        return anomaly_score, ood_gts, points, sam_outputs
