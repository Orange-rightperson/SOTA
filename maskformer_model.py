...
class MaskFormer(nn.Module):
    ...
    def __init__(
       ...
       # self.test_proposal_sampler_1=NLSSampler(thresh=-0.15, ignore_mask=None, method="kmeans", num_points=50)
        ...
        self.prompt_fusion = deepcopy(self.fusion_module)
        mask_in_chans = 16
        embed_dim = 256
        self.num = 2
        self.mask_downscaling = deepcopy(self.sam.prompt_encoder.mask_downscaling)
    ...
    def from_config(cls, cfg):
        ...
        losses =[ "sam_bce", "sam_dice"]#[ "labels", "objects" ,"masks"]#, "obj_mask", "obj_labels"]["contrastive",
    ...
    def sam_output(self, x, query_coordinates,  uncertainty, o_feature, road_prediction):
        ...
        for i in range(x.shape[0]):
            ...
            ood_embeddings = self.mask_downscaling(mask_prompt)
            extra_embeddings = self.road_attention(road_mask)
            #print(torch.max(extra_embeddings))
            #print(torch.min(extra_embeddings))
            
            dense_embeddings = ood_embeddings * extra_embeddings 
            dense_embeddings,_ = self.prompt_fusion(feat_list=[ood_embeddings, dense_embeddings])
