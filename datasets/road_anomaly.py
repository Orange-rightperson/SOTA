...
class RoadAnomaly(Dataset):
    ...
    def __init__(self, hparams, transforms):
        ...
        for i in range(self.num_samples):
            label_path = os.path.join(hparams.dataset_root, 'frames', self.images[i][:-4] + '.labels',
                                      'labels_semantic.png')
            road_path = os.path.join(hparams.dataset_root, 'roads', self.images[i][:-4]+'_mask.png')
    ...
    def __getitem__(self, index):
        ...
        road = cv2.imread(self.roads[index], cv2.IMREAD_GRAYSCALE)
        label = label[:, :, 0]
        label[label == 2] = 1

        aug = self.transforms(image=image, mask=label)
        r = self.transforms(image=image,  mask=road)
        image = aug['image']
        label = aug['mask']
        road = r['mask']
        ...
        return image, label.type(torch.LongTensor)#, road.type(torch.LongTensor)
